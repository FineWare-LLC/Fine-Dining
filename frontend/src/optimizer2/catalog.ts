// @ts-nocheck
import { MAX_RECIPES } from './constants';
import { MealModel } from '@/models/Meal/index';
import { RecipeModel } from '@/models/Recipe/index';

const FALLBACK_UPDATED_AT = '2025-01-01T00:00:00.000Z';
...
function createEmptyMealCatalogError() {
    const error = new Error('Meal catalog is empty. Add meals before optimizing.');
    error.code = 'emptyMealCatalog';
...
async function ensureDbConnection() {
    ...
    dbConnectFn = dbModule.dbConnect;
    return dbConnectFn();
}
...
function fallbackCatalog(error, request) {
    const baseRecipes = [
        {
            ...
            cuisine: ['mediterranean', 'breakfast', 'vegetarian'],
            mealType: 'BREAKFAST',
        ...
        },
        {
            ...
            cuisine: ['mexican', 'dinner', 'vegetarian'],
            mealType: 'DINNER',
    ...
    for (const recipe of baseRecipes) {
        if (recipe.allergens.some(a => allergenSet.has(a))) {
            excluded.allergens += 1;
            continue;
        }

        if (recipe.ingredients.some(ing => bannedSet.has(ing.ingredientId))) {
            excluded.banned += 1;
            continue;
        }

        if (desiredCuisine.length > 0 && !desiredCuisine.some(c => recipe.cuisine.includes(c))) {
            excluded.cuisine += 1;
            continue;
        }

        if (vegetarianOnly) {
            const isVegetarian = recipe.cuisine.includes('vegetarian') || recipe.cuisine.includes('vegan');
            if (!isVegetarian) {
                excluded.cuisine += 1;
                continue;
            }
...
export async function fetchRecipeCatalog(request) {
    ...
    if (meals.length === 0) {
        throw createEmptyMealCatalogError();
    }
