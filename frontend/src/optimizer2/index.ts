...
const EMPTY_FILTERED_CATALOG_ERROR_MESSAGE = 'No recipes available after applying filters.';
const EMPTY_FILTERED_CATALOG_FAILURE_MESSAGE = 'No recipes available after applying filters. Broaden your catalog filters and try again.';

function buildOptimizationFailureResponse({
    modelHash = null,
    solveMs = null,
    ...
    if (objectiveWeights) {
        objective.breakdown = { ...objectiveWeights };
    }
    return {
        status: 'error',
        message,
...
export async function optimizeMealPlan(payload, options = {}) {
    ...
    if (cached && cached.status === 'optimal') {
        return {
            ...cached,
            diagnostics: cached.diagnostics ? { ...cached.diagnostics } : cached.diagnostics,
        };
    }
        ...
        const isEmptyMealCatalog = error?.code === 'emptyMealCatalog';
        const isEmptyFilteredCatalog = error?.message === EMPTY_FILTERED_CATALOG_ERROR_MESSAGE;
        const response = buildOptimizationFailureResponse({
            modelHash,
            solveMs,
            objectiveWeights: normalized.objectiveWeights,
            ...(isEmptyMealCatalog
                ? {
                ...
                : isEmptyFilteredCatalog
                    ? {
                        message: EMPTY_FILTERED_CATALOG_FAILURE_MESSAGE,
                        warnings: [
                            EMPTY_FILTERED_CATALOG_ERROR_MESSAGE,
                            'Optimization failed before a plan could be produced.',
                        ],
                    }
                : {}),
        });
