...
export const addRecipeToCookbook = withErrorHandling(async (_, { cookbookId, entry }, context) => {
    ...
    if (Object.prototype.hasOwnProperty.call(validatedEntry.input, 'recipeVersion')) {
        cookbookEntry.recipeVersion = validatedEntry.input.recipeVersion;
    }

    cookbook.entries.push(cookbookEntry);
    try {
        await cookbook.save();
    } catch (error) {
