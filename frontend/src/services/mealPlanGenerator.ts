...
function filterMealsByDiet(meals, user) {
    ...
    if (!childRestrictionProfile.valid) {
        throw childRestrictionProfile.error;
    }

    const userAllergies = collectCanonicalAllergies(user);
    const disallowedIngredients = collectCanonicalDisallowedIngredients(user);
    const dietaryPattern = typeof user?.questionnaire?.dietaryPattern === 'string'
        ? user.questionnaire.dietaryPattern.trim().toUpperCase()
        : '';
    const hardRestrictions = childRestrictionProfile.hardRestrictions;
    ...
    return meals.filter(meal => {
        ...
        if (candidateHasIngredientConflict(meal, disallowedIngredients)) {
            return false;
        }
    ...
    try {
        ...
        const { data: mealsData } = await apolloClient.query({
            ...
            variables: { 
                ...
                allergenExclusions: userAllergies.size > 0 ? Array.from(userAllergies) : undefined,
 ...
 * By default this returns an empty array on lookup failure so existing callers stay safe.
 */
