...
export default function CrawlerAdmin() {
    ...
    const fetchAll = useCallback(async () => {
        try {
            ...
            if (recipeRes.ok) setRecipeStatus(await recipeRes.json());
        ...
        <ProtectedRoute allowedRoles={['ADMIN']}>
            ...
            >
                <Box sx={{ maxWidth: 1180, mx: 'auto' }}>
                    ...
                    ) : !recipeStatus || !restaurantStatus ? (
                        <Alert severity="error">
                            Crawler API is not reachable at {crawlerApi}.
                        </Alert>
                    ) : (
                        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1.2fr' }, gap: 2.5 }}>
                            <Paper sx={panelSx()}>
                                ...
                                <Box sx={{ display: 'flex', gap: 1.5, mb: 2 }}>
                                    <Button
                                        ...
                                        disabled={acting || recipeStatus.running}
                            ...
                            <Paper sx={panelSx()}>
                                ...
                                <Button
                                    variant="contained"
                                    startIcon={<PlayArrowIcon />}
                                    disabled={acting || selectedSourceIds.length === 0}
