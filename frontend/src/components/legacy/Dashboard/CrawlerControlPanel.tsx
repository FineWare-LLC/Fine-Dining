...
const CRAWLER_API =
    process.env.NEXT_PUBLIC_CRAWLER_BACKEND ||
    process.env.NEXT_PUBLIC_CRAWLER_API ||
    'http://localhost:8001';
...
export default function CrawlerControlPanel() {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            const [recipeRes, restaurantStatusRes, sourceRes] = await Promise.all([
                ...
                fetch(`${CRAWLER_API}/restaurant-crawler/status`),
                fetch(`${CRAWLER_API}/restaurant-crawler/sources?include_aggregators=${includeAggregators}`),
            ]);
            ...
            setFetchError(`Crawler API is not reachable at ${CRAWLER_API}.`);
            setRecipeStatus(null);
            setRestaurantStatus(null);
            setSources([]);
        } finally {
    ...
    }, [includeAggregators, selectedSources.length]);
    ...
    const startRecipeCrawler = async () => {
        ...
        setMessage('');
        try {
            const res = await fetch(`${CRAWLER_API}/crawler/start`, {
                method: 'POST',
                headers: authHeaders(),
            });
    ...
    const stopRecipeCrawler = async () => {
        ...
        setMessage('');
        try {
            const res = await fetch(`${CRAWLER_API}/crawler/stop`, {
                method: 'POST',
                headers: authHeaders(),
            });
            const data = await res.json();
    ...
    const addRecipeUrls = async () => {
        ...
        try {
            const res = await fetch(`${CRAWLER_API}/crawler/add-urls`, {
                method: 'POST',
                headers: authHeaders(),
    ...
    const runRestaurantCrawler = async () => {
        ...
        try {
            const res = await fetch(`${CRAWLER_API}/restaurant-crawler/run`, {
                method: 'POST',
                headers: authHeaders(),
                body: JSON.stringify({
                    source_ids: selectedSourceIds,
                    dry_run: dryRun,
                    limit_per_source: Number(limitPerSource) || 40,
                    include_aggregators: includeAggregators,
                }),
            });
            const data = await res.json();
