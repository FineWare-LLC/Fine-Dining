...
import { OverpassPayloadError, OverpassProvider } from '../../services/providers/OverpassProvider';
...
describe('OverpassProvider', () => {
    ...
    it('should reject malformed payloads when elements is not an array', async () => {
        const mockData = {
            elements: null
        };
        ...
        await assert.rejects(
            ...
            (error) => {
                assert.ok(error instanceof OverpassPayloadError);
                assert.strictEqual(error.message, 'Invalid Overpass response payload');
                return true;
            }
        );
    });
