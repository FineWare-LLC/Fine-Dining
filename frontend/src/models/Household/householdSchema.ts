...
const getEffectiveServingMultiplier = (entry) => {
    if (!entry || typeof entry.servingMultiplier !== 'number' || !Number.isFinite(entry.servingMultiplier)) {
        return 1;
    }

    return entry.servingMultiplier;
};

const computeHouseholdHeadcount = (members = [], guests = []) => {
    const memberServings = Array.isArray(members)
        ? members.reduce(
            (total, member) => total + (member.includeInPlanning ? getEffectiveServingMultiplier(member) : 0),
            0,
        )
        : 0;
    const guestServings = Array.isArray(guests)
        ? guests.reduce((total, guest) => total + getEffectiveServingMultiplier(guest), 0)
        : 0;

    return Math.max(1, Math.ceil(memberServings + guestServings));
};
