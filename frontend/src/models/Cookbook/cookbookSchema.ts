...
const COOKBOOK_ENTRY_ERROR_MESSAGES = {
    ...
    invalidRecipeVersion: 'Please enter a valid recipe version timestamp.',
};
...
const normalizeOptionalDate = (value, defaultValue) => {
    if (value === undefined || value === null || value === '') {
        return { hasValue: false, value: defaultValue };
    }
    ...
    const parsedDate = value instanceof Date
        ...
        : new Date(typeof value === 'string' ? value.trim() : value);
...
export function validateCookbookEntryInput(input) {
    ...
    const recipeVersion = normalizeOptionalDate(input.recipeVersion, null);
    if (recipeVersion.hasValue && recipeVersion.value === null) {
        return {
            valid: false,
            input: null,
            error: createCookbookEntryValidationError('invalidRecipeVersion'),
        };
    }
    ...
    if (recipeVersion.hasValue) {
        validatedInput.input.recipeVersion = recipeVersion.value;
    }
