...
import { validateUserSearchInput } from '../../utils/userSearch.ts';
...
/**
 * Search for users based on a safe text query that matches name or email.
 *
...
export async function searchUsers(query) {
    const validation = validateUserSearchInput(query);

    if (!validation.valid) {
        throw validation.error;
    }

    return this.find(validation.input.filter).sort(validation.input.sort);
}
