...
const NearbyRestaurants = () => {
    ...
    const loadingFeedback = buildNearbyRestaurantsFeedbackState({
        isLoading: loading,
        message: userLocation ? 'Finding nearby restaurants...' : 'Getting your location...',
    });
    const errorFeedback = buildNearbyRestaurantsFeedbackState({
        state: 'error',
        message: error || 'Could not load nearby restaurants. Please try again.',
    });
    const emptyFeedback = buildNearbyRestaurantsFeedbackState({
        state: 'empty',
        message:
            'No restaurants found in this area. Try increasing the search radius or selecting a different location.',
    });
    const successFeedback = buildNearbyRestaurantsFeedbackState({
        state: 'success',
        message: `Found ${places?.length || 0} restaurants within ${formatDistance(radius)}`,
    });
    ...
    return (
        ...
        >
            ...
            {loading && (
                <Box
                    role={loadingFeedback.role}
                    aria-live={loadingFeedback.ariaLive}
                    ...
                    sx={{
                        ...
                        alignItems: 'center',
                        py: 4,
                        minHeight: loadingFeedback.minHeight,
                    }}
                >
                    <CircularProgress size={40} />
                    <Typography variant="body2" sx={{ ml: 2 }}>
                        {loadingFeedback.message}
                    </Typography>
                </Box>
            )}
