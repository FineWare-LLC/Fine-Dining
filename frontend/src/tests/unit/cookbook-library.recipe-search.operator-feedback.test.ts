...
test('buildRecipeSearchFeedbackState exposes loading, empty, success, and error states with stable spacing', () => {
    ...
    const fallbackErrorState = buildRecipeSearchFeedbackState({ recipes: undefined });
    assert.equal(fallbackErrorState.state, 'error');
    assert.equal(fallbackErrorState.actionKind, 'retry-search');
    assert.ok(fallbackErrorState.error instanceof Error);
...
test('recipes page keeps recipe search feedback shells and resolved announcements accessible', () => {
    assert.match(recipesPageSource, /recipeSearchFeedback\.state === 'loading'/);
    assert.match(recipesPageSource, /recipeSearchFeedback\.state === 'empty'/);
    ...
    assert.match(recipesPageSource, /buildRecipeSearchFeedbackContainerStyles\(recipeSearchFeedback\.state\)/);
